...
public class MyViewPagerAdapter extends PagerAdapter {
    ...
    private List<PagerItemView> itemViews = new ArrayList<>();

    public MyViewPagerAdapter(Context context) {
        mContext = context;
        itemViews.clear();
        for (int i = 0; i < 4; i++) {
            itemViews.add(new PagerItemView(context));
        }
    }
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        PagerItemView itemView = itemViews.get(position);
    ...
    public List<PagerItemView> getItems() {
        return itemViews;
    }
}
